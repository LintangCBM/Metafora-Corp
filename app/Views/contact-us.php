<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link href="<?= base_url('images/Logo.png') ?>" rel="icon" />
  <title>Metafora Corp - Contact Us</title>
  <link rel="stylesheet" href="<?= base_url('css/style.css') ?>" />
  <link
    href="https://fonts.googleapis.com/css2?family=Work+Sans:wght@300;400;500;600;700&display=swap"
    rel="stylesheet" />
</head>

<body>
  <!-- Header -->
  <header>
    <div class="logo">Metafora Corp</div>
    <nav>
      <a href="<?= site_url('home') ?>">Home</a>
      <span>|</span>
      <a href="<?= site_url('blog') ?>">Blog</a>
      <span>|</span>
      <a href="<?= site_url('who-we-are') ?>">Who We Are</a>
      <span>|</span>
      <a href="<?= site_url('corporate-responsibility') ?>">Corporate Responsibility</a>
      <span>|</span>
      <a href="<?= site_url('contact-us') ?>" style="font-weight: bold; border-bottom: 2px solid">Contact Us</a>
    </nav>
    <div class="button">
      <button class="login-btn">Login</button>
      <button class="language-btn">
        <span>&#127760;</span>
      </button>
    </div>
  </header>

  <!-- Main -->
  <main>
    <!-- Description Section -->
    <section class="description">
      <h1>Connect With Us</h1>
      <p>
        We love to hear from you, lets connect with us to collaborate
        something project or make something partnership business.
      </p>
      <div class="buttons">
        <button>Contact Us</button>
      </div>
    </section>

    <!-- Maps Section -->
    <section class="map">
      <div>
        <img src="../images/mapping.png" alt="Map Location" />
      </div>
      <div class="map-content">
        <h2>Our Maps</h2>
        <p>
          Wonorejo, Sariharjo, Ngaglik, Sleman Regency,<br />Special Region of
          Yogyakarta 55581
        </p>
        <button>Find Us</button>
      </div>
    </section>

    <!-- Contact Section -->
    <section class="contact">
      <div class="container">
        <div class="content">
          <h2>Our Contact</h2>
          <p>Say hi to us, or make schedule to talk with us.</p>
          <div class="buttons">
            <button>www.metaforacorp.co.id</button>
            <button>info@metaforacorp.co.id</button>
            <button>+62 896 6423 0232</button>
          </div>
        </div>
        <div class="icons">
          <img src="../images/email.png" alt="Email" />
          <img src="../images/call.png" alt="Phone" />
          <img src="../images/browser click.png" alt="Web" />
        </div>
      </div>
    </section>

    <!-- Social Media Section -->
    <section class="social-media">
      <div class="container">
        <div class="icons">
          <img src="../images/instagram.png" alt="Instagram" />
          <img src="../images/linkedin.png" alt="LinkedIn" />
          <img src="../images/facebook.png" alt="Facebook" />
          <img src="../images/x.png" alt="Twitter" />
        </div>
        <div class="content">
          <h2>Follow Us on Social Media</h2>
          <p>
            Be first people most up to date with us on Social Media, sometimes
            we share Quotes, Our Milestone and some Promo.
          </p>
          <div class="buttons">
            <button>Instagram</button>
            <button>Twitter</button>
            <button>Facebook</button>
            <button>LinkedIn</button>
          </div>
        </div>
      </div>
    </section>

    <!-- Career Section -->
    <section class="career">
      <div class="container">
        <div class="content">
          <h2>CAREER IN METAFORA CORP</h2>
          <button>Join With Us</button>
        </div>
        <img src="../images/growth career.png" alt="Career Icon" />
      </div>
    </section>
  </main>

  <!-- Footer -->
  <footer class="footer">
    <div class="footer-container">
      <div class="footer-section">
        <h4>About Us</h4>
        <p>
          Metafora Corp is a Holding Company, our main business is
          technologies services, property development & retail fashion.
        </p>
        <p>© 2021-2025</p>
      </div>
      <div class="footer-section">
        <h4>Website Services</h4>
        <ul>
          <li><a href="#">Website Development</a></li>
          <li>
            <a href="#">Portfolio Website Development</a>
          </li>
          <li>
            <a href="#">Price List Website Development</a>
          </li>
        </ul>
        <h4>Product ERP</h4>
        <ul>
          <li><a href="#">Enterprise Resources Planning</a></li>
        </ul>
      </div>
      <div class="footer-section">
        <h4>Our Company</h4>
        <ul>
          <li><a href="#">Insight About Us</a></li>
          <li>
            <a href="#">Telephone: +62 896 6423 0232</a>
          </li>
          <li>
            <a href="#">Email: info@erporio.com</a>
          </li>
        </ul>
        <h4>Our Social Media</h4>
        <ul>
          <li>
            <a href="#" target="_blank">Instagram</a>
          </li>
          <li><a href="#" target="_blank">LinkedIn</a></li>
          <li><a href="#" target="_blank">Twitter</a></li>
          <li><a href="#" target="_blank">Facebook</a></li>
        </ul>
      </div>
      <div class="footer-section">
        <h4>Metafora Corporation</h4>
        <ul>
          <li><a href="#">Deriota IOT Developer</a></li>
          <li><a href="#">ERPORIO ERP Software</a></li>
          <li><a href="#">Societech Startup Enabler</a></li>
          <li><a href="#">ERPORI ERP Consultant</a></li>
          <li>
            <a href="#">Mahafidz Information Technology Consultant</a>
          </li>
          <li>
            <a href="#">Metafora Business Consultant</a>
          </li>
        </ul>
      </div>
    </div>
    <div class="footer-bottom">
      <p>© 2025 Metafora Corporation</p>
    </div>
  </footer>
</body>

</html>